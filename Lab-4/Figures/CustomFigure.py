...
class CustomFigure(Figure):
   ...
   def setup(self):
      self.size = self._bindVertexData(self.vertex_buffer_id, np.array(self.verticies, dtype=np.float32))
      ...
      if len(self.lineVertices) > 0:
         self.lineSize = self._bindVertexData(self.line_buffer_id, np.array(self.lineVertices, dtype=np.float32))
